...
test('the AI settings page shows the configured defaults and the OpenRouter catalogue', function () {
    ...
    $this->actingAs(User::factory()->admin()->create())
        ->get(route('admin.ai.edit'), ['X-Inertia' => 'true', 'X-Inertia-Partial-Component' => 'admin/AiSettings', 'X-Inertia-Partial-Data' => 'models', 'X-Inertia-Version' => app(Middleware::class)->version(request()) ?? ''])
        ->assertOk()
